...
def is_client_message(msgType: int) -> bool:
    """
    Returns whether or not the given message type is a Client message
    """

    return msgType >= CLIENT_HELLO and \
           msgType <= CLIENT_OBJECT_LOCATION

def is_state_server_message(msgType: int) -> bool:
    """
    Returns whether or not the given message type is a State Server message
    """

    return msgType >= STATESERVER_CREATE_OBJECT_WITH_REQUIRED and \
           msgType <= STATESERVER_OBJECT_DELETE_CHILDREN

def is_database_state_server_message(msgType: int) -> bool:
    """
    Returns whether or not the given message type is a Database State Server message
    """

    return msgType >= DBSS_OBJECT_ACTIVATE_WITH_DEFAULTS and \
           msgType <= DBSS_OBJECT_DELETE_DISK

def is_database_server_message(msgType: int) -> bool:
    """
    Returns whether or not the given message type is a Database Server message
    """

    return msgType >= DBSERVER_CREATE_OBJECT and \
           msgType <= DBSERVER_OBJECT_DELETE

def is_client_agent_message(msgType: int) -> bool:
    """
    Returns whether or not the given message type is a Client Agent message
    """

    return msgType >= CLIENTAGENT_SET_STATE and \
           msgType <= CLIENTAGENT_DONE_INTEREST_RESP

def is_message_director_message(msgType: int) -> bool:
    """
    Returns whether or not the given message type is a Message Director message
    """

    return msgType >= CONTROL_CHANNEL and \
           msgType <= CONTROL_CLEAR_POST_REMOVES

def is_net_messenger_message(msgType: int) -> bool:
    """
    Returns whether or not the given message type is a Net Messenger message
    """
